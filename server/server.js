...
// GET /todos/{id}
// id var is on the request object
app.get('/todos/:id', (req, res) => {
  ...
  // to test the GET in Postman
  // res.send(req.params);

  // gets the todo for particular ID
  Todo.findById(id).then((todo) => {
    ...
    if (err) {
      res.status(400).send();  //err); - not sending error since might contain private info
    }
  });
});
